...
class SimpleSerialTransmitter(Node):
    def __init__(self):
        ...
        self.declare_parameter("port", "/dev/cu.usbserial-130")
        ...
        self.arduino_ = serial.Serial(
            port=self.port_, baudrate=self.baudrate_, timeout=0.1
        )

    def msgCallback(self, msg):
        self.get_logger().info(
            "New message received, publishing on serial: %s" % self.arduino_.name
        )
